...
class FandomPage(object):
    ...
    def __load(self, redirect=True, preload=False):
        ...
        if (not redirect) and ('redirects' in query):
            ...
            raise PageError(self.pageid if self.pageid else None,
    ...
    def source(self):
        ...
        sources = soup.find_all(self.selector[0], {'class': self.selector[1]})
        print(sources)
    ...
    def revision_id(self):
        ...
        if not getattr(self, '_revid', False):
            ...
            request = _wiki_request(query_params)
            self._revision_id = request['query']['pages'][str(
    ...
    def images(self):
        ...
        if not getattr(self, '_images', False):
            ...
            if 'images' in request['query']['pages'][str(self.pageid)]:
                images = [image['title'] for image in request['query']
                          ['pages'][str(self.pageid)]['images']]
            else:
                images = []
            ...
            if images != []:
                ...
                images = [page['imageinfo'][0]['url']
                          for page in request['query']['pages'].values() if 'imageinfo' in page]
            ...
            for section in sections:
                if section_title is None:
                    ...
                    if 'sections' in section:
                        section_text += get_section_recursive(
                            section['sections'])
